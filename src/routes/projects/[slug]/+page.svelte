...
<SEO
	title={`${data.metadata.title} - Vinayak Dev`}
	description={data.metadata.description}
	keywords={data.metadata.tech ?? []}
	image={data.metadata.image}
/>

{#if data.metadata.image}
	<div class="w-full aspect-[2.5/1] rounded-xl overflow-hidden bg-gray-200 dark:bg-gray-700 mb-8">
		<img
...
{:else}
	<div class="w-full aspect-[2.5/1] rounded-xl bg-gray-200 dark:bg-gray-700 mb-8 flex items-center justify-center text-gray-400 dark:text-gray-500 text-sm">
		Featured image coming soon
	</div>
{/if}

<div class="flex flex-wrap gap-2 mb-6">
	{#each (data.metadata.tech ?? []) as tag}
		<span class="text-xs font-medium px-2.5 py-1 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300">{tag}</span>
	{/each}
</div>
